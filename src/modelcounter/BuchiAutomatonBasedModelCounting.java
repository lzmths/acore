...
public class BuchiAutomatonBasedModelCounting<S> {

	private FieldMatrix<BigFraction> T = null;
	private Automaton<S, BuchiAcceptance> automaton = null;
	private LabelledFormula formula = null;
    private Object[] states = null;
    private boolean countFirstTime = false;
	//public static int TIMEOUT = 300;

	public BuchiAutomatonBasedModelCounting(LabelledFormula formula) {
...
//
		automaton = (Automaton<S, BuchiAcceptance>) translator.apply(formula);
...
//		automaton = (Automaton<S, ParityAcceptance>) translator.apply(formula);
	...
	public  void countOnlyFirstTime() {
		countFirstTime = true;
	}
	...
	public  BigInteger count(int bound) {
		...
		try {
			...
			System.out.println("BuchiAutomatonBasedModelCounting::count TIMEOUT.");
		}
		catch (InterruptedException | ExecutionException e) {
			System.err.println("BuchiAutomatonBasedModelCounting::count ERROR. " + e.getMessage());
		}
		return BigInteger.ZERO;
	}
